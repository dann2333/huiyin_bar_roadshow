...
class TavernOrchestrator:
    ...
    async def generate_receipt(self, session_id: str) -> dict:
        ...
        receipt_prompt = (
            ...
            f"请为一张 3 英寸 x 3 英寸的知乎风格分享卡生成一段短箴言，给路演现场的路人带走。\n\n"
            f"⚠️ 重要规则：\n"
            f"1. 不要使用任何真实用户名，也不要出现任何角色名或说话者标签\n"
            f"2. 只输出箴言正文，不要标题、署名、日期、编号、列表、Markdown 或任何装饰符号\n"
            f"3. 输出 1 到 2 句，总长度控制在 52 个中文字符以内，最多不超过 64 个中文字符\n"
            f"4. 句子要像可转发的知乎短句：具体、有余味、温暖克制，不要鸡汤化，不要说教\n"
            f"5. 禁止写成引语或动作描写，不要出现“轻声说”“说道”“提醒”“总结”等叙述前缀\n"
            f"6. 不要解释，不要换行超过 1 次\n\n"
            f"今晚的对话：\n{context}"
        )
