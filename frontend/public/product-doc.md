...
| 项目部署与测试     | danuonuo               |
| 背景音乐生成       | SUNO V5 Beta           |
| 人声生成           | MiniMax speech-2.8-hd  |
| 刘看山酒保图片生成 | Gemini 3.1 Flash Image |
| 项目开发平台       | SecondMe               |

> 学生团队，课程较满，如有开发不到位的地方，请各位见谅。
> 项目架构较为简单，主要为了表达我们的心中所想。

> 感谢SecondMe提供的平台和比赛机会！！！
> 感谢知乎提供的IP授权和API支持！！！

### 宇宙版权声明

本项目所有知乎相关的 IP 以及密钥均是官方赛内授权，因此本项目只用于以下比赛：

![知乎 × SecondMe 全球 A2A 黑客松](/images/赛事图片.jpg)

**该项目不以商业为目的，用途仅限于参加本次比赛，考虑到 IP 版权问题，项目暂时没有完整开源打算（预期赛后会进行去除 IP 等操作后再开源），未获得本团队允许和知乎官方授权不得抄袭搬运，以及用于各种商业用途！！！**
...
**同路人共鸣** — 「当初」的大牛不是在回忆过去，而是正处于和用户一样的迷茫中。他活在那个时间点，不知道未来会怎样，和用户是真正的同路人。

**时间维度启发** — 「如今」的大牛用岁月沉淀后的视角补充现实感悟。两个版本的对话，帮用户从时间维度理解选择的本质。

**沉浸式体验** — 深夜酒馆场景 + 角色扮演 + 实时流式对话 + 刘看山 IP 加持。不是冷冰冰的问答，而是一场有温度的深夜对话。
...
| AI 引擎 1   | SecondMe           | -         | OAuth2 + SSE 流式对话（默认引擎）           |
| AI 引擎 2   | Qwen（通义千问）   | qwen-plus | OpenAI 兼容接口（定制模型，需在设置中配置） |
...
- config.py —— 配置管理（SecondMe / Qwen / 知乎，运行时更新 + 脱敏 + 语言设置）
- main.py —— FastAPI 入口（CORS 配置 + 路由挂载）

**前端（frontend/src/）**

- App.tsx：主应用组件（状态管理 + SSE 处理 + UI 渲染 + 引擎切换 + 文档弹窗 + 设置面板 + 蝴蝶效应弹窗 + 欢迎页 + 音效系统）
- i18n.ts：多语言翻译表（中/英 70+ key）
- index.css：全局样式系统（暗黑酒馆主题 + 角色色彩 + 动画 + 蝴蝶效应弹窗 + 欢迎页）
- hooks/useSSEStream.ts：SSE 流式数据 Hook（通用 SSE 消费逻辑）
- hooks/use-background-music.ts：背景音乐管理 Hook（播放/暂停/音量/自动播放）
...
- 按五幕剧结构编排对话流程，控制幕间转场
- 调度 AI 引擎（SecondMe 默认，定制模型备选）生成角色回复
- 管理自动对话模式的启停和轮次控制
...
**定制模型（Qwen 等）—— 需在设置中配置 API Key 后启用**

- 接口格式：OpenAI Chat Completions 兼容
- 端点：默认 DashScope，可自定义
- 上下文方式：多轮 messages 数组
- 优势：独立于 SecondMe，无需 OAuth2，多轮对话理解更好
- 配置：通过设置面板或 QWEN_API_KEY 环境变量启用

**切换机制：**

- 前端点击“定制模型”时先查询 qwen_available，未配置则提示用户去设置中填写 API
- 通过 POST /api/tavern/engine 发送切换请求
...
- 设置相关：settingsOpen / settingsData / settingsMsg
- 蝴蝶效应弹窗：butterflyOpen / butterflyInput
- 欢迎页：welcomeDismissed（localStorage 持久化）
- 音乐：isPlaying / volume / musicPanelOpen
- 分享：receiptText / shareStatus / shareUrl
- 文档：docOpen / docContent
...
由于接口的限制，以及技术水平限制，我们无法做出完整符合我们预期的项目。

如果未来允许，我们计划：

> 1. 能够深层次的读取所有大 V 的知乎数据，构建更加单一，更加深刻的大 V 分身 agent。
> 2. 能够给被选取的大 V 发私信，告诉大 V，昨晚他的经历给了谁启发，让遇到相同问题的无名之辈，能够有与大 V 交流的机会。
> 3. 我们希望能更好的和刘看山这个 IP 结合起来，做一些交互的动画，让产品的体验感更好。
